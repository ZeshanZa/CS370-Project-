...
function ProjectsPage() {
    ...
    const handleAddProjectSubmit = async (e) => {
        ...
        try {
            await axios.post('http://3.91.27.166:8000/projects/create/', projectData, { //I decided to remove local variable since some of yall are havin issues so just put URL instead 
                headers: {
                    'Authorization': `Token ${token}`,
    ...
    const handleEditSubmit = async (e) => {
        ...
        const token = localStorage.getItem('access_token');
        try {
            await axios.put(`http://3.91.27.166:8000/projects/${editingProjectId}/`, {
                title: editTitle,
